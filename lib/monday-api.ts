...
const BOARD_ID = process.env.MONDAY_PRODUCTS_BOARD_ID || '9944534259'
...
export class MondayAPIService {
  ...
  async getProducts(): Promise<Product[]> {
    try {
      ...
      const products: Product[] = board.items_page.items.map((item: { id: string; name: string; column_values: { id: string; text?: string; value?: string }[] }) => {
        const columnValues = item.column_values.reduce((acc: Record<string, string | undefined>, col: { id: string; text?: string; value?: string }) => {
  ...
  private parseNumber(value: string | unknown): number {
